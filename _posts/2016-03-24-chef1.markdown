...
so, here´s header template and we call it.. well, header.rb

```ruby
<header value = "<%= @info %>" />
```

and another template app.headed.config.erb , we incude header using render directive. see the first line of the exmaple
...
sidenote- if we're doing cross-platform solutions, we might need to have different configs for different platforms

it can be done using attribute
...
just like a bosch. see how paramerers are passed to definition
...
if you can spot code duplication here, you're wrong. filename and path are configured in parameter _path_ and that string that goes afer _createfile_ is just name of the step.
...
for that open Berksfile and add cookbook dependency,  our good fellow from  previous execrices
...
and patch metadata.rb file with dependency
...
this is how you can use recipes from other cookbooks (inside default.rb or any other recipe)
...
1) create folder for testing

```shell
mkdir -p /tmp/berks/cookbooks
```

2) populate test folder with all the cookbooks and their dependencies
...
3) create file client.rb in test folder
...
those books would be taken from supermarket
